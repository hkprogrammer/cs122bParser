...
BEGIN
    ...
    DECLARE existing_max_genre INT;
    SELECT id INTO existing FROM genres WHERE name = genre_name LIMIT 1;
    IF existing IS NULL THEN
		SELECT max(id) INTO existing_max_genre FROM genres;
        IF existing_max_genre IS NULL THEN
			SELECT 100 INTO existing_max_genre;
        END IF;
        INSERT INTO genres(id,name) VALUES (existing_max_genre+1,genre_name);
        SET existing = existing_max_genre+1; 
    END IF;
	COMMIT;
